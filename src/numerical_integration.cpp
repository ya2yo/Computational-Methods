...
// Compute the trapezoidal estimate at refinement level k.
double Romberg::trapezoid(int k) {
    double range = sup - inf;
    int n = 1 << k;
    double h = range / n;
    if (k == 0) return range / 2.0 * (f(inf) + f(sup));
    double sum_new = 0.0;
...
void Romberg::printTable() const {
    cout << "\n--- Romberg integration table ---" << endl;
    for (int j = 0; j <= printn; ++j) {
        cout << "R[" << j << "]: ";
        for (int i = 0; i <= j; ++i) cout << R[j][i] << " ";
        cout << endl;
    }
    cout << "----------------------\n" << endl;
}
...
double Romberg::solve(int max_k, double tolerance) {
    ...
    for (int k = 0; k < max_k; ++k) {
        ...
        if (k > 0 && abs(R[k][k] - R[k - 1][k - 1]) < tolerance) {
            printn = k;
            return R[k][k];
        }
    }
